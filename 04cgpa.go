...
func studentByCGPA(cgpa int) ([]StudentData, error) {
	var students []StudentData

	rows, err := db.Query("SELECT * FROM student_Data WHERE cgpa < ?", cgpa)
	if err != nil {
		return nil, fmt.Errorf("error1: no student found with cgpa < %d, %v,  ", cgpa, err)
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("error3 : no student found with cgpa < %d, %v,  ", cgpa, err)
	}
	return students, nil
}
